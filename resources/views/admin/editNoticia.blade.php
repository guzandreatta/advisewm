...
<head>
	...
	<script type="text/javascript">
        $(document).ready(function() {    
			...
			$('#noticias').on('change', function(e){
				e.preventDefault();				
				var indexSelectedNews = $("#noticias").prop('selectedIndex');
				if (indexSelectedNews == 1){
					$("#subirOrden").prop('disabled', true).css('opacity',0.5);
				} else {
					$("#subirOrden").prop('disabled', false).css('opacity',1);
				}
				if ($("#noticias option:last").is(":selected")){
			...
			$("#subirOrden").on('click', function(e){
				...
				var selectedNews = $('#noticias').val();				
				var url = '{{url("subirOrdenDeNoticia")}}';
				$.ajax({
					type: "POST",
					url: url,
					dataType: "json",
					data: JSON.stringify(selectedNews),
					success: function( msg ) {	
						$("#noticias").empty().append(msg.noticias);
						$('#noticias option[value="'+selectedNews+'"]').prop('selected', true)
			...
			$("#bajarOrden").on('click', function(e){
				var selectedNews = $('#noticias').val();				
				var url = '{{url("bajarOrdenDeNoticia")}}';
				$.ajax({
					type: "POST",
					url: url,
					dataType: "json",
			...
			$('#idioma').on('change', function(e){
				...
				$("#reorder").hide();
				$("#editNews").hide();
				var url = '{{url("obtenerNoticiasPorOrden")}}';
				$.ajax({
					type: "POST",
					url: url,
		...
		function editarNoticia(){
			...
			var url = '{{url("obtenerNoticiaPorId")}}';
			$.ajax({
				type: "POST",
				url: url,
				data: JSON.stringify(selectedNews),
				...
				success: function( msg ) {	
					...
					$("#editNews").show();
					// console.log(msg.noticia[0]);
					// console.log(msg.multimedia);
					$('input[name=id]').val(msg.noticia[0].Id);
					$('input[name=idiomaid]').val(msg.noticia[0].IdiomaId);
					$('input[name=title]').val(msg.noticia[0].Titulo);
					$('input[name=desc]').val(msg.noticia[0].Descripcion);
					$('#summernote').summernote('code', msg.noticia[0].Texto);
					$('select[name=order]').val(msg.noticia[0].Orden);
					$('select[name=active]').val(msg.noticia[0].Activo);
					
					if (msg.noticia[0].FechaInicio != null){
						var initialDate = getDateFromString(msg.noticia[0].FechaInicio);
						if (initialDate != null){
					...
					if (msg.noticia[0].FechaFin != null){
						var endDate = getDateFromString(msg.noticia[0].FechaFin);
						if (endDate != null){
...
<script>
  ...
  ga('send', 'pageview');
	...
	<div id="ajaxResponse"></div>
		...
		@endif
		<div class="form-group">
			<label for="noticias">{{ __('messages.select_news') }}:</label>
			<select class="form-control" id="noticias">
				<?php echo($noticias); ?>
			</select>
		</div>	
	...
	<form id="editNews" role="editarNoticia" action="subirNoticiaEditado" method="POST" enctype="multipart/form-data" style="display:none;">
		...
		<div class="form-group">
		  ...
		  <div class="form-group">
			  ...
			  <div>
				<img id="currentHomeImage" alt="Current home image" height="360">
			  </div>			  
		  ...
		  <div class="form-group">
			  ...
			  <div>
				<img id="currentCoverImage" alt="Current cover image" height="360">
			  </div>
			  <span>{{ __('messages.change_image') }}</span>
			  <input type="file" class="form-control" id="coverimage" name="coverimage" accept="image/*">
		  </div>
		  <div class="form-group">
			  <label for="news">{{ __('messages.news') }}:</label>
			  <br/>
			  <textarea class="form-control" id="summernote" name="news">{{ old('news') }}
				</textarea>
		  </div>
		<div class="form-group">
